...
var instrucciones = ['Utilizando el teclado', 'Mueve la pieza al casillero vacío'];

// Arreglo para ir guardando los movimientos que se vayan realizando

var movimientos = [];
var movimientosMezcla = 0;
...
function agregarUltimoMovimiento (ultimoMovimiento) {
  ...
  var numeroAnterior = 0;
    ...
    for (var i = 0; i < grilla.length; i++) {
      ...
      for (var j = 0; j < fila.length; j++) {
        ...
        if (numeroAnterior > columna) {
          gano = false;
        } 

        numeroAnterior = columna;
      }
    }
...
function mostrarCartelGanador() {
  var cartel = document.getElementById("exito");
  cartel.style.display = "block";
  var resultado = document.getElementById("cantidad");
  resultado.innerText = 'En solo ' + cantMovimientos + ' movimientos!';
} 

// Muestra la flecha después de que se mezclaron las piezas
function mostrarFlecha (direccion) {
...
// Cuenta cuántos movimientos se hicieron
function contarMovimientos() {
  cantMovimientos = movimientos.length - movimientosMezcla;
}
...
function mezclarPiezas(veces) {
  ...
  movimientosMezcla = movimientos.length;
